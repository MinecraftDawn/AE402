...
class Ball(pygame.sprite.Sprite):  #球體角色
    ...
    def update(self):  #球體移動
        radin = math.radians(self.direction)
        self.dx = self.speed * math.cos(radin) # 水平位移
        self.dy = -1 * self.speed * math.sin(radin)
        

        self.rect.x += self.dx  # 移動球體
        self.rect.y += self.dy
        
        if self.rect.left <= 0 or self.rect.right >= screen.get_width() - self.image.get_width() // 2:
            self.bouncelr()
        elif self.rect.top <= self.image.get_height()//2:
            self.rect.top = self.image.get_height() // 2
            self.bounceup()
            
        if self.rect.bottom >= screen.get_height() - self.image.get_height() // 2:
            return True
        else:
            return False
